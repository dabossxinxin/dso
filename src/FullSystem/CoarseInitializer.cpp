...
namespace dso
{
	...
	bool CoarseInitializer::trackFrameV2(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
	{
		...
		for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--)
		{
			...
			Vec8f bu, bv;
			resetPoints(lvl);
			//Vec3f resOld = calcResAndGS(lvl, Hu, Hw, Hv, bu, bv, refToNew_current, refToNew_aff_current, false);
		}
		return true;
	}
	...
	Vec3f CoarseInitializer::calcResAndGSV2(
		...
		AffLight refToNew_aff, bool plot)
	{
		...
		for (int i = 0; i < npts; i++)
		{
			...
			for (int idx = 0; idx < patternNum; idx++)
			{
				...
				float u = pt[0] / pt[2];						// Px/Pz
				float v = pt[1] / pt[2];						// Py/Pz
				float Ku = fxl * u + cxl;
				float Kv = fyl * v + cyl;
				float new_idepth = point->idepth_new / pt[2];	// dpi/Pz
				...
				// 若找到的模板上的点的灰度值NaN则说明当前特征位置notGood
				...
				dp0[idx] = new_idepth * dxInterp;						// drk/drho[0]
				dp1[idx] = new_idepth * dyInterp;						// drk/drho[1]
				dp2[idx] = -new_idepth * (u*dxInterp + v * dyInterp);	// drk/drho[2]
				dp3[idx] = -u * v*dxInterp - (1 + v * v)*dyInterp;		// drk/dtheta[0]
				dp4[idx] = (1 + u * u)*dxInterp + u * v*dyInterp;		// drk/dtheta[1]
				dp5[idx] = -v * dxInterp + u * dyInterp;				// drk/dtheta[2]
				dp6[idx] = -hw * r2new_aff[0] * rlR;					// drk/daj
				dp7[idx] = -hw * 1;										// drk/dbj
				dd[idx] = dxInterp * dxdd + dyInterp * dydd;			// drk/ddpi
				r[idx] = hw * residual;									// rk
			...
			// 当patternNum数量大于8时单独计算SSE无法快速计算的部分
	...
	void CoarseInitializer::propagateDown(int srcLvl)
	{
		...
		for (int i = 0; i < nptst; i++)
		{
			...
			// TODO:为什么父点方差小的时候点不可用
			if (!parent->isGood || parent->lastHessian < 0.1) continue;

			// 若点OK则根据权重融合父点以及当前点的逆深度
			// 若点NG则直接将其父点的逆深度赋给当前点
			if (!point->isGood)
			{
	...
	void CoarseInitializer::makeNN()
	{
		...
		// 搜索每一层金字塔中特征点的最邻近点以及在上一层金字塔中的父节点
		for (int lvl = 0; lvl < pyrLevelsUsed; lvl++)
		{
			Pnt* pts = points[lvl];
			int npts = numPoints[lvl];
