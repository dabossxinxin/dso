...
namespace dso
{
	...
	double PointFrameResidual::linearize(CalibHessian* HCalib)
	{
		...
		{
			state_NewState = ResState::INLIER;
		}
	...
	void PointFrameResidual::debugPlot()
	{
		...
		{
			if (state_state == ResState::INLIER) cT = Vec3b(255, 0, 0);
	...
	void PointFrameResidual::setState(ResState s) 
	{ 
		state_state = s;
	}
	...
	void PointFrameResidual::resetOOB()
	{
		...
		state_NewState = (ResState::OUTLIER);
		setState((ResState::INLIER));
	}

	void PointFrameResidual::printRows(std::vector<VecX> &v, VecX &r, int nFrames, int nPoints, int M, int res)
	{
	...
	void PointFrameResidual::applyRes(bool copyJacobians)
	{
		if (copyJacobians)
		{
			...
			if (state_NewState == ResState::INLIER)// && )
			{
